...
Lexer Lexer_new(const byte* input, const u32 inputLength) {
    return (Lexer) { .reader = Reader_new(input, inputLength) };
}
...
Token Lexer_next(Lexer* lexer) {
    ...
    if (isDigit(b)) {
        ...
        type = WORD;
        for (Reader_next(&lexer->reader); Reader_hasNext(&lexer->reader) && isWord(Reader_peek(&lexer->reader)); Reader_next(&lexer->reader));

        const Position end = lexer->reader.position;
        if (start.line == end.line) {
            const u32 length = end.column - start.column;
            if (b == 'l' && length == 3 && memeq("let", lexer->reader.input, 3)) {
                keywordType = LET;
            } else if (b == 'f' && length == 8 && memeq("function", lexer->reader.input, 8)) {
                keywordType = FUNCTION;
            } else if (b == 's' && length == 6 && memeq("struct", lexer->reader.input, 6))  {
                keywordType = STRUCT;
            }
        }
    } else if (b == '"') {
